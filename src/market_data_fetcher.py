...
2. ``CalendarSpreadData`` — the three book frames (front leg, back leg,
   spread instrument) *time-synced onto a common index*, plus their symbols.

3. ``CalendarSpreadContractSpec`` — the static CME parameters needed to turn
...
class CalendarSpreadData:
    """Time-synced order books for the two legs and the spread instrument.

    All three frames follow the book-frame schema and share the same index
    (the result of ``MarketDataFetcher._sync_books``).
    """
...
class MarketDataFetcher:
    ...
    def fetch_calendar_spread_data(
        ...
        """Fetch synced books for ``symbol1`` (front), ``symbol2`` (back) and their spread.

        Each instrument is fetched separately, then the three streams are
        synced onto a common time index so books can be compared at the
        same/similar timepoints.
        """
        spread_symbol = self._build_spread_symbol(symbol1, symbol2)

        front = self._fetch_single_instrument(symbol1, time_start, time_end)
        back = self._fetch_single_instrument(symbol2, time_start, time_end)
    ...
    def fetch_calendar_spread_contract_specification(
        ...
        """Fetch the static contract parameters for the pair and its spread."""
        spread_symbol = self._build_spread_symbol(symbol1, symbol2)
        definitions = self._fetch_definitions(symbol1, symbol2, spread_symbol)
        return self._build_contract_spec(symbol1, symbol2, spread_symbol, definitions)
